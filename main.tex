...
To help visualize the differences in behavior between information gathering actions and goal-oriented actions based on hindsight optimization, Fig. \ref{heatmap1} shows heatmaps produced by discretizing a 2D space with  multiple candidate goals and scoring each state according to either expected information gain or goal-oriented reward. Fig. \ref{heatmap2} shows the same heatmap, but with a different belief state - in this figure, goals A and B are believed to be more likely than the other goals. As the figures demonstrate, the basic behavior of the two systems is similar, with higher scores close to the more likely goals. However, inspection of Fig. \ref{heatmap2} in particular shows that the goal-oriented scoring favors moving directly between the two most likely goals, where the information gathering scoring has a more even distribution around the most likely goals but also including states that remain further back from the two goals in a position that keeps the optimal control vectors for all goals different.
...
\label{exp_setup}
\includegraphics[width=\columnwidth]{figures/task_setup.JPG}
\end{figure}
