...
Recent work in shared autonomy and assistive teleoperation has moved toward more relaxed assumptions about what information the robot is given. Earlier noteworthy work assumes that the robot has been given information about the current goal or desired behavior and uses this knowledge to determine assistance strategies \cite{debus2001cooperative, goodrich2001experiments, aigner1997human}. While systems relying on such knowledge can be efficient, this information may not be available in all cases.

More recently, many studies have demonstrated assistive teleoperation systems that rely on first predicting the user intent, then using a distinct assistive strategy that assists toward the currently predicted user goal \cite{fagg2004extracting, kragic2005human, yu2005telemanipulation, dragan2012formalizing, schultz2017goal}. This framework has the benefit of providing assistance without knowledge of the user's goal, but has a drawback in that the assistance is only available once the current goal has been predicted with sufficient confidence. 

An exciting recent avenue of study that our work builds on and investigates further is the use of hindsight optimization to assist toward multiple goals simultaneously. \citet{javdani2015shared} setup a partially observable Markov decision process (POMDP) and use QMDP approximation \cite{littman1995learning} to estimate the best action at each timestep. This approximation eliminates information gathering and results in action selection that weights the assistance toward each goal by the current belief in that goal. This enables assistance without confidence in a single goal. TODO: go over hindsight optimization experiments.

\subsection{Robotic Information Gathering}

\section{Information Gathering in Shared Autonomy}
\subsection{Formulation}
Give basic information about setup and go over hindishgt optimization notation 
